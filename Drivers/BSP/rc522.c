...
////					HAL_Delay(500);
				if(Read_Block(0,blockData)==MI_OK)
				{
...
////返    回: 成功返回MI_OK
////******************************************************************/
char my_Value(unsigned char dd_mode,unsigned char addr,unsigned char *pValue)
{
    char status;
		status=PcdAuthState(0x60,addr,Password_Buffer,UID);
		if(status!=MI_OK)
    return status;
		status=PcdValue(dd_mode,addr,pValue);
    return status;
}
